...
namespace std {
template<>
struct hash< reducedsole::uuid > {
public:
    // hash functor: hash uuid to size_t value by pseudorandomizing transform
    size_t operator()( const reducedsole::uuid &uuid ) const {
        if( sizeof(size_t) > 4 ) {
...
inline bool reducedsole::uuid::operator==( const reducedsole::uuid &other ) const {
...
inline std::string reducedsole::uuid::pretty() const {
    std::stringstream ss;
